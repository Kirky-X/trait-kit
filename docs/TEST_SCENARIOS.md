...
| CFG-13 | `restore_config` 无快照 → `MissingConfig`（key 为本地化 no-snapshot 消息） | 异常 | confers | 无 | `src/kit/kit.rs::restore_returns_error_when_no_snapshot` | tests/e2e/e2e_config.rs |
